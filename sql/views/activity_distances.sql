...
AS
WITH with_routes AS (SELECT activity_id, activity_date, distance_miles, activity_type_id
    FROM activities
    JOIN activity_routes USING (activity_id)
    JOIN routes USING (route_id)),
without_routes AS (SELECT activity_id, activity_date, distance_miles, activity_type_id
    FROM activities
